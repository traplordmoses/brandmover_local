...
def _build_capabilities_section() -> str:
    ...
    return (
        ...
        "2. **Analyze step metadata** — `smart_record` returns `step_timeline` and "
        "`dead_time_hints`. USE THESE to build your edit segments (see below).\n"
        "3. **Edit** — Use `edit_video` with segments derived from the step timeline\n"
        "4. **Style** — Apply phone mockup + gradient (`apply_style: true` in edit_video)\n"
        "5. **SELF-REVIEW** — Call `review_video` on the styled output BEFORE sending\n"
        "   - If score < 7 or pass = false: re-edit and review again\n"
        "   - If duration mismatch > 3s from target: re-edit with corrected segments\n"
        "   - Check for: blank screens, stuck frames, missing key moments, loading states\n"
        "6. **Send** — Only send the video after it passes review (score >= 7, pass = true)\n\n"

        "### USING STEP METADATA TO BUILD EDIT SEGMENTS\n"
        "`smart_record` returns two key fields:\n"
        "- **step_timeline**: Array of {step, timestamp, duration, action, target, narration, error} "
        "for every action taken during recording. Timestamps are seconds from recording start.\n"
        "- **dead_time_hints**: Pre-computed suggestions for segments to cut. Each hint has "
        "{steps, action, target, total_duration, hint}.\n\n"

        "**Rules for building segments from step_timeline:**\n"
        "1. **Dead time = consecutive steps on the same page with no meaningful action change.** "
        "If steps 10-14 are all 'wait' or all 'scroll' on the same target totaling 15s, "
        "only keep timestamp[10] to timestamp[10]+1s and timestamp[14] to timestamp[14]+1s.\n"
        "2. **Loading/wait steps > 3s → cut entirely.** The viewer doesn't need to watch a spinner.\n"
        "3. **Error steps → cut entirely.** If a step has an error (element not found, timeout), "
        "that segment shows nothing useful — remove it.\n"
        "4. **Rapid swipes are GOOD content** — keep the full swipe sequence. "
        "Consecutive swipe steps should NOT be treated as dead time.\n"
        "5. **Navigation (goto) steps need ~1.5s** — keep the page transition but cut the "
        "hydration wait that follows.\n"
        "6. **First and last steps are sacred** — always include the opening (landing page) "
        "and closing (finish narration) in full.\n"
        "7. **Start with dead_time_hints** — process every hint first. Then scan step_timeline "
        "for any remaining long gaps (duration > 5s on a non-swipe step) you might need to trim.\n\n"

        "**Example:** Recording is 45s. step_timeline shows:\n"
        "- Steps 1-3 (0s-8s): goto homepage, wait 3s, click connect → KEEP 0-5s, CUT 5-8s (wait)\n"
        "- Steps 4-6 (8s-15s): wallet flow clicks → KEEP all (meaningful interaction)\n"
        "- Steps 7-9 (15s-28s): goto /swipe, wait 4s, wait 3s → KEEP 15-16.5s, CUT 16.5-28s\n"
        "- Steps 10-15 (28s-38s): swipe, swipe, swipe, swipe, swipe → KEEP ALL (hero content)\n"
        "- Steps 16-17 (38s-45s): goto gallery, finish → KEEP all\n"
        "Result segments: [[0,5], [8,15], [15,16.5], [28,38], [38,45]] → ~27.5s final video\n\n"
