...
def _build_capabilities_section() -> str:
    ...
    return (
        ...
        "<promo_video_tool>\n"
        "CRITICAL: When the user asks for a promo video, short-form video, reel, or anything involving "
        "animated text over a background video, you MUST use the `generate_promo_video` tool. "
        "NEVER use execute_code, Playwright, PIL, or any manual approach to build videos. "
        "The generate_promo_video tool handles everything: AI background generation, glassmorphism "
        "text card rendering, typewriter animation, chat bubbles, and ffmpeg compositing.\n\n"
        "Parameters:\n"
        "- title (required): Bold title text, use \\n for line breaks e.g. 'FOID\\nPRAYER'\n"
        "- conversation (required): list of {role, text} for the chat animation\n"
        "- subtitle: smaller text below title\n"
        "- background_style: liquid_metal, aurora, particle_field, smoke\n"
        "- background_color: primary color e.g. 'iridescent blue'\n"
        "- duration_seconds: video length (default 15)\n"
        "- output_filename: e.g. 'my_promo.mp4'\n"
        "- fresh_bg: true to force new AI background generation\n"
        "</promo_video_tool>\n\n"
